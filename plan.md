...
- [x] Week 3（#10 #11 #13 #14 #15）已上线（新增 4 篇指南页 + 1 篇用例页）。
- [ ] Week 4（#16 #17 #18 #19 #20）待执行。

## 基于 Vercel 数据的阶段调整（2026-03-12）
- 近 30 天 Visitors 2,183（+30%）、Page Views 3,473（+29%），说明增长方向成立。
- 但流量高度集中在 `/` 与 `/archive-instagram`，内容页已有零星进入，说明“入口强、内容分发弱”。
- Bounce Rate 74%（+1%），因此下一个阶段不应只继续扩页，还要优先强化高流量入口到 guides / use-cases 的站内导流。
- 执行调整：已把首页、Instagram 页、About、Header、Footer 补上更明确的 guides / workflow 入口；Week 4 继续做，但优先服务现有高流量入口的分发与转化。
